if (!window.bapdosa.takeOut) {
    if(!window.bapdosa.takeOut) window.bapdosa.takeOut = {};
}

$(document).ready(function(){		
	window.bapdosa.takeOut.init();
});
...
window.bapdosa.takeOut = (function() {
	var mOrderId = ""; //주문번호
	...
	function eventReg(){
		...
		//주문영역세팅
		orderArea = $("#takeout-page .list.minh table tbody");
		
		//카테고리 선택시
		$("#takeout-page .class-category-area > li").click(function(e){
			//e.preventDefault();
			...
			$("#takeout-page .food_list .foodlist ul").empty();			
			if(selCategoryId){
...
//			$(".fre_box .ui-link").click();
		...
		$(".class-event-account-go").click(function(e){
			...
			var url="/pos/order/account.do?orderId=" + mOrderId;
			
			if(saveFlag){				
				var afterurl = "/pos/order/account.do";
		...
		$(".class-event-customer-request").click(function(e){
			...
			if(!mOrderId){
				//if(confirm("추가된 메뉴가 있습니다. 저장하시겠습니까?")){
					var afterurl = "/pos/memo/customerRequest.do";
					beforeOrderSave(afterurl);	
				//	return false;
...
//			}
		...
		$("#takeout-page .paging_count > span").text(orderAreaList.size());
		$("#takeout-page .price_total > span").text(window.bapdosa.util.setComma(sumPrice));
	}
	
	function displayMenu(categoryId){
		var menuBody = $("#takeout-page .food_list .foodlist ul");
		menuBody.empty();
					...
					var li = $("<li>",{						
						...
						click : function(e){
							...
							orderMenu($(this).attr("categoryId"), $(this).attr("menuId"));
									  ...
									  .append($("<span>").text(parseInt(menuObj.TAKEOUTPRICE)/1000));
					menuBody.append(li);
				});
		...
		if(orderDetailId){
			...
			if(orderFlag){
				...
				if(selMenuObj){	
					...
					var displayPrice = selMenuObj.TAKEOUTPRICE;
					
					
					if(mIsDcPrice){
						displayPrice = selMenuObj.TAKEOUTPRICE * ((100-selMenuObj.TAKEOUTDISCOUNT)/100);
					}
					
					console.log("selMenuObj.TAKEOUTDISCOUNT: " + selMenuObj.TAKEOUTDISCOUNT);
...
/*		<tr>
	...
	function getCategoryInfoList(){
		...
		var success = function(returnJsonVO){
			...
			$(categoryInfoList).each(function(index,obj){
				//console.log(obj);
				if(index < 4){
					var selLi = $("#takeout-page .class-category-area > li:eq(" + index + ")");
			...
			var firstCategoryId = $("#takeout-page .class-category-area > li:eq(0)").attr("categoryId");
	...
	function displayTableInfo(){
		...
		var success = function(returnJsonVO){
			...
			$("#takeout-page .class-area-table-name").text(tableInfoObj.TABLENAME)
